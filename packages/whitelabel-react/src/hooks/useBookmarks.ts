...
export function useBookmarkPercentage(assetId: string): number {
  const [asset] = useAsset(assetId);
  const [bookmarks] = useBookmarks();
  const bookmark = bookmarks?.find(b => b.assetId === assetId);
  if (!bookmark || !asset) return 0;
  return Math.round((((bookmark.lastViewedOffset as number) * 100) / asset.duration) as number);
}
