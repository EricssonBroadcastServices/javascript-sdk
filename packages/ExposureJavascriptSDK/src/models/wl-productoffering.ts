...
export class WLOfferingPrice extends OfferingPrice {
  public getPriceWithVAT = (translations: Translations) => {
    const vatString = this.vat.percentage === 0 ? "" :
      ` - ${this.vat.percentage}% ${this.vat.included ?
        translations.getText("VAT_INCLUDED") :
        translations.getText("VAT_NOT_INCLUDED")
      }`;
...
export class WLProductOffering extends ProductOffering {
  ...
  public getRentalLengthAsHours = () => {
    return moment.duration(this.rentalLength).asHours();
  }
  public getRentalLengthAsDays = () => {
    return moment.duration(this.rentalLength).asDays();
  }

  public getRentalLengthString = (translations: Translations) => {
    const duration = this.rentalLength ? moment.duration(this.rentalLength) : moment.duration(this.recurrence);
    ...
    const minuteText = minutes > 1 ? translations.getText(["DATES", "MINUTES"]) : translations.getText(["DATES", "MINUTE"]);
